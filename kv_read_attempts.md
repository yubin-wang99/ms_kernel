...
- **배치 scalar regime: vpack로 재측정 → B≤8 robust win(Phase 35 정정).** GQA(32/8) B1 0.84× / B4 0.89× /
  B8 0.92–0.93× WIN, B16 ~tie(1.00–1.01×), B32 slight loss(1.02–1.04×). Phase 35의 "배치 universal
  loss(1.07–1.26×)"는 vpack 이전 결과. **ncu 정정:** B=32는 BW-bound 아님(DRAM 9.5%, L1/TEX 74.9%,
  Compute 70.9%) — compute+shared 포화에서 MSAQ의 bfe-decode가 순수 tax. cooperative-exp(중복 exp
  32×→1×)를 두 커널에 공정 적용했으나 단순한 MXINT8이 더 이득 → 고배치 비율 약간 악화(고로 B≥16 loss는
  near-fundamental). single-token은 occupancy-bound라 무영향(여전히 win). (Phase 43–44)
- **텐서코어 regime: 하드웨어 벽(3090).** native sub-byte MMA 부재로 두 포맷이 같은 bf16/int8 MMA 입력
  타일 → 바이트 이점 MMA 직전 소멸(vpack 무관). 게다가 decode 경로 아님(텐서코어 P·V는 large-M
  prefill/shared-prefix용; autoregressive decode는 M=1~small-batch라 scalar+vpack가 이미 win).
  Blackwell(native MXFP8/FP4)만 여는 과제.
