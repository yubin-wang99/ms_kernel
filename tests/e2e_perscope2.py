...
tests/harness_perscope_results2.md + .jsonl. Usage: MS_FAST=1 python tests/e2e_perscope2.py"""
...
jf = open(os.path.join(outdir, "harness_perscope_results2.jsonl"), "w")
lines = []
...
emit("Current kernels: prefill = dequant-weight + cuBLAS (ties bf16); batched decode (B>=16) = "
     "dequant+cuBLAS; KV decode = (u,gs)-specialized wide-load. ms in µs; ratio <1 = MSAQ faster.")
...
jf.close()
with open(os.path.join(outdir, "harness_perscope_results2.md"), "w") as f:
    f.write("\n".join(lines) + "\n")
print(f"\n[wrote] tests/harness_perscope_results2.md + .jsonl")
