...
### 활성화 양자화 `quant_act` — 두 커널 ([wa_gemm.cu](csrc/wa_gemm.cu))
W+A의 활성화 전처리(STAGE 0). bf16 `X[M,K]` → int8 워드 `qX[M,K]` + 블록당 E8M0 지수 `sa_exp[M,NB]`.
**한 워프가 한 `(m, blk)` 담당**(lane k = 블록의 32원소): lane이 x 로드 → `__shfl_xor`로 amax 워프리듀스
→ `ea = floor(log2 amax) − E_MAX` (E8M0 지수) → 양자화. 원소당 **정확히 1회** 양자화(메모리-bound, M=8/K=4096에서 ~13µs).

- **`quant_act_kernel`** [:53](csrc/wa_gemm.cu#L53) — **plain MXINT8** 활성화: `qX = clip(round(x·2^−ea), ±127)`.
  **MXINT8 baseline**이 쓴다.
- **`quant_act_msaq_kernel`** [:81](csrc/wa_gemm.cu#L81) — **MSAQ-s** 활성화(가중치와 같은 맨티사 공유 포맷):
  블록 base `sa=2^ea` → upper code `q_upper=clip(round(x/(sa·2^u)))` (8−u bit) → 잔차 `r=x−q_upper·sa·2^u`
  → gs-그룹 평균을 공유 code `r_shared=clip(round(mean_g(r)/sa))` (u bit) → int8 워드 `qX=q_upper·2^u+r_shared`.
  **MSAQ W+A**가 쓴다. 이는 **포맷 차이(MSAQ-s vs MXINT8 활성화)이지 최적화가 아니라서** baseline엔 안 미러됨 — [[runtime-quant-msaq-vs-mxint8]].

**누가 쓰나**: ① prefill W+A GEMM의 int8 IMMA 스테이지, ② decode W+A B=1 / B=2..15의 **split 경로**.
decode의 dequant-in-staging float GEMV는 이 `qX`를 `qX·sa`로 되돌려 float MAC에 쓴다(int8 dot 안 씀, §2 W+A 항목).
**완전 융합(`MS_WA_FUSED=1`)은 이 커널을 GEMV staging에 흡수하지만 output-블록당 ~32× 중복이라 더 느림**(§5, documented-negative).
...
- **AA prefill 어텐션 (low-bit×low-bit self-attention)** — Q·Kᵀ, P·V를 *양자화끼리* 연산. **production
  아님**(아래 이유). 호출처는 `tests/aa_kernel_bench.py`, `tests/shared_prefix_attn_bench.py`뿐.
  **3단 흐름** (`aa_kernel_bench.py` `msaq_aa()`):
  1. **`qk_wmma_kernel<U4>`** [kv_attention.cu:1218](csrc/kv_attention.cu#L1218) — `scores[M,Lk] = Q[M,D] @ K[Lk,D]ᵀ`.
     bf16 WMMA(64×64 타일). K는 양자화 평면을 shared에 dequant(`Bs[lk][d]`), Q는 shared(`As[m][d]`), D축 수축.
     출력 fp32 scores. **AA면 Q를 미리 양자화**(`Qa=qmsaq(Q)`)해 넣어 low×low.
  2. **causal softmax** — `[H,L,L]` scores를 torch가 마스킹+softmax → P(bf16). **AA면 P도 양자화**(`qmsaq(P)`).
  3. **`pv_wmma_kernel<U4>`** [:1096](csrc/kv_attention.cu#L1096) — `out[M,D] = P[M,Lk] @ V[Lk,D]`.
     V 양자화 평면 dequant(`Bs[d][k]`), P는 shared(double-buffer 파이프, unpack(next)가 MMA(cur)에 겹침),
     split-K(grid.z). (+`qk_wmma_mx`/`pv_wmma_mx` = MXINT8판, `qk_scalar` = 비-텐서코어 변형.)

  **왜 production이 아닌가:** ① **prefill** — `[H,L,L]` scores를 materialize하고 **L×L softmax 행렬 P를 양자화
  = O(L²) 작업**(tax 0.4→11.7ms)을 해야 해서 **bf16 fused flash(SDPA)에 2.1–2.7× 짐**(L=512–2048). fused flash는
  이 materialize를 통째로 회피. ② **decode** — production `kv_decode_wide_kernel`은 Q를 bf16으로 읽음(decode는
  KV 바이트에 memory-bound, Q는 [B,Hq,hd]로 극소) → Q 양자화해도 latency 이득 0, **S6(AA)=S5 같은 커널**.
  즉 AA는 **정확도 비용(~+0.9–1.0pp PPL)이지 latency 비용이 아님**. (정확도 frontier가 u2에 고정 → u4 nibble
  불가 → MXINT8도 못 이김. 측정: [precision/aa_attn_results.md](precision/aa_attn_results.md),
