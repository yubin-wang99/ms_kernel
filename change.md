...
# Phase 46 — Online K-rotation kernel (Hadamard H₁₂₈): 정확도 win을 ≈무비용 latency로 실현

**배경(정확도 선행, commit 7a37da2).** block-Hadamard 회전이 MSAQ 정확도에 도움 되는지 QSNR+소표본
PPL로 검증한 결과, 가치가 **KV-Key에 집중**됨을 확인(weight는 미미, u4엔 오히려 해). Key는 persistent
**채널 outlier**가 있어 full head_dim(H₁₂₈) 회전이 이를 섞어(QuaRot 메커니즘) 죽인다 → **공격적 nibble
u4/gs2를 robust로**(KV u4/mg8 PPL +5.14% FAIL → +2.04%). 그 u4/gs2가 정확히 vpack KV-decode 커널이
이기는 config라, 회전은 **robust frontier를 가장 빠른 config 쪽으로 확장**한다. 단 회전은 latency-bound
decode hot-path 비용이라 TPOT delta를 정확도 이득과 저울질해야 함(no-fake-win). 본 phase는 그 커널을
짜고 추가 latency를 측정.

**커널.** 회전은 pair op라 `(Q·H)(K·H)ᵀ = Q·Hₙ·Hₙᵀ·Kᵀ = Q·Kᵀ`(orthonormal `Hₙ=H/√D`)로 score 보존
→ K를 양자화 직전 회전 + Q를 QKᵀ 전 미러. **FWHT**(7 butterfly stage = 128·7, naive 128² matmul 회피).
- **standalone**(`csrc/rotate.cu`, `torch.ops.msaq.hadamard_rotate`): 측정용 상한. 정확성 rel-err 2.6e-3
  vs `x@(hadamard/√128)`, QKᵀ 보존 3.3e-3.
- **fused**: K회전을 `kv_append`에 통합(`kv_append_rot_kernel`/`torch.ops.msaq.kv_append_rot` — head당
  한 block이 full D-row 적재→shared FWHT→NB 스레드가 동일 token-major 슬롯에 quant+pack); Q회전을 attn
  prologue에 통합(`kv_decode_wide_kernel`, `MS_KV_QROT=1` gating — q_sh[D] 로드 직후 FWHT 1회/block).
  V는 회전 안 함(정확도 무관).

**latency(RTX 3090, `tests/rot_{kv,fused}_bench.py`, `precision/rot_kv_latency.md`).**
- **standalone은 launch-bound**: ~9µs/launch로 (B,Lk)에 **flat**. 결정적 증거 — Q+K 한 launch(8.9µs)
  == K 단독(9.0µs) == 두 launch(18µs)의 절반 → 비용은 전부 **per-launch 오버헤드**(40~1280행×128이
  launch를 못 채움), math 아님. attn 대비 비율은 최악(B=1/Lk1024 28%, attn 31µs)뿐, 현실 배치/긴
  컨텍스트선 <1~5%.
- **fused는 진짜 marginal ≈ 0**: `kv_append_rot` − `kv_append` = **−0.03µs(노이즈)**; `MS_KV_QROT=1`
  − 0 = **≤~1µs/≤~1%**(attn run-to-run jitter 이하, 음수 항목은 노이즈). standalone ~9µs는 전부 launch
  오버헤드였고 fuse로 제거됨.

**정확성(격리 검증, 양자화 노이즈가 버그를 못 가리도록).** (A) 동일 회전-K 캐시에서 커널-q회전 vs
호스트-q회전 → rel_fro **1.5e-3**(bf16/FWHT 순서차만). (B) `append_rot` 캐시 vs 호스트
`pack_kv(bf16(K)@H)` 디코드 → rel_fro **0.0**(byte-exact: 커널 fp32 FWHT가 호스트 matmul과 양자화
코드까지 일치). `test_kv` 72/72, 기본 경로 불변(qrot=0, append 미변경).

**end-to-end PPL 검증(`precision/rot_qrot_ppl.py`, `rot_results.md §3`).** 기존 정확도 수치는
**effective-dequant fold**(`msaq(K@H)@Hᵀ/D`, H unnormalized, D=2⁷가 E8M0에 정확히 fold)로 Q 미회전.
배포 커널은 **orthonormal `Hₙ=H/√D`**의 진짜 online 경로라 `1/√128`(비-pow2)이 per-32-block E8M0를
미세 이동시켜 fold와 bit-identical하지 않음 → 직접 검증. wikitext-2/Llama-3.1-8B(BF16 PPL 6.5684):
online이 fold win을 재현 + **오히려 미세하게 더 좋음**(online ≤ fold 전부, 0.01~0.20pp). headline
**KV u4/mg8 +5.14%(FAIL) → +1.89%(robust)**가 실제 online Q회전+rotated-K-quant에서도 성립.

**공정성(`for_fair_comparison.md` §(D)).** 회전은 MSAQ-K만 적용(MXINT8 미러 안 함) → W+A 활성화
...
A variant that keeps the shared code SIGN-FREE. Trick: make the residual one-sided by rounding the upper
TOWARD -INF (math floor = the Euclidean-division quotient; NOT truncation toward zero). round-to-nearest
leaves a +/- residual; round-toward-(-inf) leaves a >=0 residual. Then the group mean is unsigned and the
recombine degenerates from an arithmetic ADD (with carry) to a pure bit-OR (concatenation).
PRECISION NOTE on "floor": round-toward-(-inf) makes the UPPER code q_un go negative for x<0 -- that is
fine, q_un is the SIGNED upper and is stored signed. What is unsigned is the RESIDUAL res = x - q_un*s_un,
and res >= 0 because q_un = floor(x/s_un) => q_un*s_un <= x. On the integer grid this is Euclidean division
by 2^u: quotient q_un = floor(q8/2^u) (can be negative), remainder res = q8 mod 2^u in [0,2^u) (least
non-negative residue, >=0 by definition). It must be Euclidean/floor, NOT C truncation toward zero -- whose
remainder takes the dividend's sign (res<=0 for x<0), breaking unsignedness. Unambiguous bit form on a
two's-complement byte: q_un = q8 >> u (ARITHMETIC, sign-extending right shift), res = q8 & (2^u - 1) (mask
yields the non-negative remainder automatically).
...
ENCODE (per block of BLOCK=32, group gs=mg)
  ...
  res    = x - q_un * s_un                                   # in [0, s_un) -> ALWAYS >= 0 (= q8 mod 2^u, the
                                                            #   non-negative remainder; incl. negatives)
...
WHY THE 4 PARTS MATTER (vs the naive one-liner "subtract and store unsigned")
  1. ROUND UPPER TOWARD -INF (Euclidean quotient; NOT trunc toward zero) -> residual is the non-negative
     remainder (q8 mod 2^u, >=0) -> the share can be genuinely sign-free. (The quotient q_un still goes
     negative for x<0; that is the SIGNED upper, stored signed -- it is the REMAINDER that is unsigned.)
     round-to-nearest would give a signed +/- residual; truncation toward zero a sign-following one --
     neither is storable unsigned without an offset/bias or a separate sign bit (see MSAQ-signed above).
...
TRADEOFF vs MSAQ-signed
  Combine is cheaper (OR-concat, no carry) and the share needs no sign plane. But round-toward-(-inf) drops
  round-to-nearest on the upper bits (a systematic +s_un/2 bias) AND the unsigned residual can only correct
  UPWARD within the rounded-down cell -- it cannot borrow downward the way the signed two's-complement share
  does. That
