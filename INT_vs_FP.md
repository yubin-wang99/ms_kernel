...
### 3a. 가장 중요한 사실: **MXFP8 KV 커널은 없다**

`pybind.cpp`를 보면 KV에 등록된 커널은 두 계열뿐이다:
...
`msfp8_*` 함수(`pybind.cpp:78-87`)는 **전부 weight 경로**(`msfp8_dequant_bf16`,
`msfp8_gemv_wide`, `msfp8_gemv_batched`, `msfp8_gemm`)다. **KV용 msfp8 커널은 존재하지 않는다**
(`grep -ni "fp8" csrc/kv_attention.cu` → FP8 원소 디코드 없음; line 515/770의 `ldexpf`는
E8M0 스케일·AA Q-quant용이지 FP8 원소 디코드가 아님).

→ **MXFP8 KV는 정밀도 실험(PPL)에만 존재**한다. `precision/msaq_mxfp8_ppl.py`는 `_kv_sdpa`로
SDPA를 가로채 K,V를 `msaq_mxfp8(...)`로 fake-quant(quantize→dequantize)할 뿐, 융합 어텐션
커널은 아니다 (`msaq_mxfp8_ppl.py:248-252`).
...
|---|---|---|
| 융합 어텐션 커널 | **있음** (`kv_decode_attention` 등) | **없음** (PPL fake-quant만) |
