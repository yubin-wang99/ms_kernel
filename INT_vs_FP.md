...
→ **KV에서 FP8는 비용(ALU)만 늘고 정확도 이득이 없어 (원래) 커널화되지 않았다.** weight 경로와 정반대로,
KV의 배포 방향은 V까지 int8로 환원하는 **"더 INT스럽게"** 쪽이다.

### 3f. 측정: MSAQ-vs-MSAQ KV decode (INT vs FP8, 같은 u/gs)

이후 FP8 KV 커널을 추가한 뒤 **동일 (u,gs)** 에서 직접 측정(`tests/msfp8_kv_decode_bench.py`,
sm_120, Hq32/Hkv8/D128/Lk4096, batched). 같은 (u,gs)면 plane 바이트·DRAM 트래픽이 **동일**하므로
차이는 순수하게 decode-ALU + V-처리(INT int8-staged vs FP8 float P·V):

| u/gs | B | INT-MSAQ | FP8-MSAQ | ratio(FP8/INT) |
|---|---|--:|--:|--:|
| u3/gs16 | 1 | 57 µs (100 GB/s) | 135 µs (43 GB/s) | **2.35×** |
| u3/gs16 | 32 | 1612 µs (114 GB/s) | 3455 µs (53 GB/s) | **2.14×** |
| u2/gs16 | 32 | 1659 µs (131 GB/s) | 3609 µs (60 GB/s) | **2.18×** |
| u3/gs8 | 32 | 1709 µs (113 GB/s) | 3689 µs (52 GB/s) | **2.16×** |

**FP8-MSAQ KV decode는 INT-MSAQ 대비 일관되게 ~2.1–2.4× 느리다** (정확도는 양쪽 rel_fro ~1.6e-3로
동등). INT가 ~115 GB/s로 이미 DRAM-roofline 한참 아래(L1TEX/ALU/occupancy-bound)인데, FP8의 추가
`ldexpf`/지수분리 + int8-staging 불가로 인한 float P·V가 유효 BW를 절반으로 깎는다. §0의 비트·바이트
동일성과 §3c의 분석이 정량적으로 확인됨 — **KV에서 FP8는 포맷 대칭성 목적의 커널일 뿐, 성능상으로는 INT가
명확히 우월하다.**
