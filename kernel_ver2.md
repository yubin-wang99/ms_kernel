...
>
> **갱신(Phase 41–42).** 위 "u4 tie"는 ncu 재측정으로 병목이 dequant-throughput이 아니라 **Pass-2
> staged-V의 L1/TEX shared/occupancy**임이 드러나며 정정됐다. `u4/gs2` + separated-scale K dot
> (`MS_KV_SEPSC`) + **packed-transposed nibble V staging(`MS_KV_VPACK`, 디폴트)** 스택으로 **single-token
> decode가 best-vs-best WIN — 같은 wide 커널이 Hq/Hkv로 MHA·GQA 둘 다**: MHA 0.83–0.97×, **GQA(32/8,
> Llama-3.1-8B 구성) 0.90–0.93×**. (중간 int8-staged V `MS_KV_V8`/transposed-padded `MS_KV_VT`는 vpack에
> 흡수돼 fallback.) GQA long-context도 win(vpack의 smem↓→occupancy↑). design-A KV-reuse 전용 커널은
> occupancy-bound documented negative. 상세 [kv_read_attempts.md] §5, [tests/kv_pack_results.md].
...
### 2.4 KV cache dequantize / attention  (memory-bound이론 / 실측 shared·occupancy-bound → **Phase 41–42: MHA·GQA decode win**)
...
**남은 정공법(Phase 34 시점).** KV read 공정 win은 GQA-amortized 재작성이 전제로 보였다 →
**그 라운드 공정 최선 = u4 tie.** (상세 [for_fair_comparison.md] Phase 34.)

**해결(Phase 41–42).** 진짜 병목은 GQA-amortize(점유율/DRAM)가 아니라 **wide 커널 Pass-2의 staged-V
L1/TEX shared 트래픽·occupancy**였다(ncu: DRAM 20.7%, L1/TEX 60–63%). `u4/gs2`에 (1) **separated-scale
K dot**(`MS_KV_SEPSC`: `s·(2^u·Σq·up+Σ_g sh·qg)`, qg 1회 precompute) (2) **packed-transposed nibble V
staging**(`MS_KV_VPACK`, 디폴트: V를 int8로 복원하지 않고 packed nibble 그대로 shared에 transposed
staging, 2 keys/thread로 write-race 회피, Pass-2 int32당 8코드 register-decode → smem 13 vs 16.5 KB로
occupancy↑)를 쌓아 **single-token decode가 MHA·GQA 모두 best-vs-best WIN**(bit-exact): MHA 0.83–0.97×,
**GQA(32/8) 0.90–0.93×**. 같은 wide 커널이 Hq/Hkv로 둘 다 처리. (중간 int8-staged V `MS_KV_V8`/
transposed-padded int8 `MS_KV_VT`는 vpack에 흡수돼 fallback.) ncu 정직성: vpack의 win은 shared-
transaction 감소(예측)가 아니라 occupancy/smem 효과였다(트랜잭션 평탄). design-A KV-reuse 전용 커널은
occupancy-bound로 미달(documented negative). 상세 [kv_read_attempts.md] §5, [tests/kv_pack_results.md].
