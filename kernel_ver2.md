...
>
> **갱신(Phase 41).** 위 "u4 tie"는 ncu 재측정으로 병목이 dequant-throughput이 아니라 **Pass-2
> staged-V의 L1/TEX shared-transaction**임이 드러나며 정정됐다. `u4/gs2` + int8-staged V(`MS_KV_V8`)
> + separated-scale K dot(`MS_KV_SEPSC`) + **transposed-padded V staging(`MS_KV_VT`)** 스택으로
> **single-token MHA decode가 0.97× best-vs-best WIN**(H8/H16, Lk4096/16384). GQA(Hq>Hkv)는 wide가
> KV를 query-head당 4× 중복 read해 parity(1.01–1.07×). 상세 [kv_read_attempts.md] §5,
> [tests/kv_pack_results.md].
...
### 2.4 KV cache dequantize / attention  (memory-bound이론 / 실측 shared-traffic-bound → **Phase 41: MHA decode 0.97× win**)
...
**해결(Phase 41).** 그러나 진짜 병목은 GQA-amortize(점유율/DRAM)가 아니라 **wide 커널 Pass-2의
staged-V L1/TEX shared-transaction**이었다(ncu: DRAM 20.7%, L1/TEX 63.5%). `u4/gs2`에 (1) **int8-
staged V**(`MS_KV_V8`: Pass-2를 MXINT8과 동일 1 int8 read로) (2) **separated-scale K dot**
(`MS_KV_SEPSC`: `s·(2^u·Σq·up+Σ_g sh·qg)`, qg 1회 precompute) (3) **transposed-padded V staging**
(`MS_KV_VT`: `pV8[blk·32·CH+kd·CH+kk]`, CH=chunk+4 → Pass-2가 int32당 4코드 conflict-free read,
4× 적은 shared txn)를 쌓아 **single-token MHA decode 0.97× best-vs-best WIN**(bit-exact). u4/gs≤2
디폴트 on. GQA(Hq>Hkv)는 wide가 KV를 query-head당 4× read해 1.01–1.07× parity(미해결). 상세
[kv_read_attempts.md] §5, [tests/kv_pack_results.md].
