...
those parameters, mark the inner loop `#pragma unroll`, and add a dispatch table for the hot values. **Done for
all four u<4 decode kernels** — the unpack is factored into a shared `template<int U_,int GS_,typename F>` device
helper (`ms_stream_block_uspec`, per-element callback, needs `--expt-extended-lambda`) reused by the W-only,
batched, and both W+A specialized kernels: measured 1.3–1.6× across `wa_gemv`/`wonly_gemv_batched`/`wa_gemv_batched`
(see `packing_explained.md` §13).
...
`#pragma unroll`을 달고, 핫 값들의 dispatch 테이블을 추가하면 됩니다. **u<4 decode 커널 4개 모두 적용 완료** —
언팩을 공유 `template<int U_,int GS_,typename F>` device 헬퍼(`ms_stream_block_uspec`, 원소별 콜백,
`--expt-extended-lambda` 필요)로 묶어 W-only·배치·W+A 특수화 커널이 재사용합니다: `wa_gemv`/`wonly_gemv_batched`/
`wa_gemv_batched`에서 1.3–1.6× 측정(`packing_explained.md` §13).
