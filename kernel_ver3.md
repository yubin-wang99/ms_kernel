...
| 완전 2-pass attention (WMMA Q·K) | **tie~loss(best ~1.0)** | Q·K(D-contraction 짧음)가 unpack-bound loss → P·V win을 희석 |
| scalar Q·K (bf16-staging 제거) | ratio win(0.78~0.92)이나 **best-vs-best tie** | 텐서코어 미사용 → 2~3× 느린 영역; MSAQ 최선은 여전히 WMMA(tie) |
...
- **scalar Q·K도 시도(Phase 40):** bf16-staging 없는 thread-per-key scalar Q·K는 ratio를 win(0.78~0.92)
  으로 뒤집지만 텐서코어 미사용이라 2~3× 느린 영역 → **best-vs-best는 tie**(MSAQ 최선은 여전히 WMMA Q·K).
  "fast=텐서코어=bf16 staging=tie / win=느린 scalar 영역" 동시 불가.
- **진짜 남은 가능성:** native sub-byte tensor-core(하드웨어 미지원)뿐.
